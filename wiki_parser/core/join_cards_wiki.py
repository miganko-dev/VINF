...
def extract_base_pokemon(name: str) -> str:
    ...
    prefixes = [
        'detective', 'dark', 'light', 'shiny', 'shadow', 'team',
        "alolan", "galarian", "hisuian", "paldean",
        'primal', 'mega', 'gigantamax', 'origin forme', 'radiant'
    ]

    suffixes = [
        'v', 'vmax', 'vstar', 'ex', 'gx', 'lv.x', 'lvx', 'lv x',
        'prime', 'legend', 'break', 'tag team', 'star',
        'g', 'gl', 'fb', 'e4', 'c', '4', 'sp',
        'delta species', 'crystal type'
    ]
...
def build_text_index(wiki_text: Dict[str, str], pokemon_names: List[str]) -> Dict[str, Set[str]]:
    ...
    for title, text in wiki_text.items():
        ...
        found_bases = words_in_text & base_names_set
...
def build_title_index(wiki_titles: List[str]) -> Dict[str, Set[str]]:
    ...
    for title in wiki_titles:
        ...
        words = set(word_pattern.findall(title_lower))
        for word in words:
...
def join_pokemon_wiki(
    pokemon_cards: Dict[str, List[Dict]],
    wiki_titles: List[str],
    wiki_info: Dict[str, Dict],
    text_index: Dict[str, Set[str]] = None
) -> List[Dict]:
    ...
    for title in wiki_titles:
        ...
        wiki_exact[title_lower] = title
        wiki_normalized[title_norm] = title
    ...
    for pokemon_name, cards in pokemon_cards.items():
        ...
        if base_norm in title_word_index:
            for title in title_word_index[base_norm]:
                if not title.startswith("List of") and len(title) < 50:
                    matched_wikis.add(title)
        if pokemon_norm in title_word_index:
            for title in title_word_index[pokemon_norm]:
                if not title.startswith("List of") and len(title) < 50:
                    matched_wikis.add(title)

        if text_index and pokemon_name in text_index:
            for wiki_title in text_index[pokemon_name]:
                matched_wikis.add(wiki_title)
        ...
        def sort_key(title: str) -> int:
            ...
            if base_lower != name_lower:
                if title_lower == base_lower:
                    return 0
                if title_lower == f"{base_lower} (pokémon)" or title_lower == f"{base_lower} (pokemon)":
                    return 1

            if title_lower == name_lower:
                return 2
            if title_lower == f"{name_lower} (pokémon)" or title_lower == f"{name_lower} (pokemon)":
                return 3

            if base_lower != name_lower and title_lower.startswith(base_lower):
                return 4
...
def save_results(results: List[Dict], stats: JoinStats) -> None:
    ...
    all_file = JOINED_OUTPUT_DIR / OutputFiles.POKEMON_WITH_WIKI_JSON
    with open(all_file, 'w', encoding='utf-8') as f:
        json.dump(results, f, ensure_ascii=False, indent=2)
    logger.info(f"Saved {len(results)} Pokemon to {all_file}")

    matched = [r for r in results if r['wiki_pages']]
    matched_file = JOINED_OUTPUT_DIR / OutputFiles.POKEMON_MATCHED_WIKI_JSON
    with open(matched_file, 'w', encoding='utf-8') as f:
        json.dump(matched, f, ensure_ascii=False, indent=2)
    logger.info(f"Saved {len(matched)} matched Pokemon to {matched_file}")
    ...
    stats_file = JOINED_OUTPUT_DIR / OutputFiles.JOIN_STATS_JSON
...
def run_join() -> JoinStats:
    ...
    text_index = None
    if wiki_text:
        text_index = build_text_index(wiki_text, list(pokemon_cards.keys()))
    ...
    logger.info(f"  Pokemon: {stats.total_pokemon}")
    logger.info(f"  Cards: {stats.total_cards}")
    logger.info(f"  Wiki pages: {stats.total_wiki_pages}")
    logger.info(f"  Pokemon with wiki: {stats.pokemon_with_wiki} ({stats.match_rate:.1f}%)")
    logger.info("-" * 40)
...
class WikiToPokemonJoiner:
    def run(self):
        ...
        for title in wiki_titles:
            wiki_data.append({
                'wiki_title': title,
                'pokemon_matches': wiki_pokemon.get(title, [])
            })

        ensure_directories()

        all_file = JOINED_OUTPUT_DIR / OutputFiles.WIKI_WITH_POKEMON_JSON
        with open(all_file, 'w', encoding='utf-8') as f:
            json.dump(wiki_data, f, ensure_ascii=False, indent=2)

        matched = [w for w in wiki_data if w['pokemon_matches']]
        matched_file = JOINED_OUTPUT_DIR / OutputFiles.WIKI_MATCHED_POKEMON_JSON
        with open(matched_file, 'w', encoding='utf-8') as f:
            json.dump(matched, f, ensure_ascii=False, indent=2)
        ...
        return {
            'total_wiki_pages': len(wiki_data),
            'wiki_with_pokemon': len(matched)
        }


def run_wiki_to_pokemon_join():
    joiner = WikiToPokemonJoiner()
    return joiner.run()
