...
from pyspark.sql.functions import col, lower, regexp_extract, length
...
class WikiSparkProcessor:
    def __init__(self, spark: Optional[SparkSession] = None):
        ...
        logger.info(f"Loaded {len(self.set_names)} set/series names")
        logger.info(f"Loaded {len(self.pokemon_names)} Pokemon names")
    ...
    def extract_page_data(self, df: DataFrame) -> List[Dict]:
        ...
        rows = df.collect()
        results = [{"title": row.title, "text": self._extract_text(row.value)} for row in rows]
        logger.info(f"Extracted content from {len(results)} pages")
        return results

    def calculate_statistics(self, pages: List[Dict]) -> Dict:
        if not pages:
            return {"total_pages": 0, "avg_text_length": 0, "max_text_length": 0, "min_text_length": 0}
    ...
    def save_results(self, pages: List[Dict], stats: Dict) -> List[str]:
        ...
        with open(titles_file, 'w', encoding='utf-8') as f:
            json.dump([p["title"] for p in pages], f, ensure_ascii=False, indent=2)
